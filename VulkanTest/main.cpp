...
private:
    ...
    void createDescriptorSetLayout() {
        ...
        if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
            throw std::runtime_error("failed to create destriptor set layout!");
        }
    }
    ...
    void createCommandPool() {
        QueueFamilyIndices queueFamiliIndices = findQueueFamilies(physicalDevice);

        VkCommandPoolCreateInfo poolinfo{};
        poolinfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
        poolinfo.queueFamilyIndex = queueFamiliIndices.graphicsFamily.value();
    ...
    void createCommandBuffers() {
        ...
        for (size_t i = 0; i < commandBuffers.size(); i++) {
            ...
            if (vkEndCommandBuffer(commandBuffers[i]) != VK_SUCCESS) {
                throw std::runtime_error("failled to record command buffer!");
            }
        }
    }
    ...
    VkShaderModule createShaderModule(const std::vector<char>& code) {
        ...
        VkShaderModule shaderModeule;
        if (vkCreateShaderModule(device, &createInfo, nullptr, &shaderModeule) != VK_SUCCESS) {
            throw std::runtime_error("failed to create shader module!");
        }

        return shaderModeule;
    }
